...
int main(int argc, char* argv[]) {
  ...
  while (getline(in_file_, line)) {
    ...
    if (sensor_type.compare("L") == 0) {
      ...
      VectorXd raw_meaasurement = VectorXd(2);
      double px;
      double py;
      iss >> px;
      iss >> py;
      iss >> timestamp;
      raw_meaasurement << px, py;

      measurement_pack_list.push_back(
              MeasurementPackage(raw_meaasurement, MeasurementPackage::LASER, timestamp)
      );
      ...
      // read measurements at this timestamp
      VectorXd raw_meaasurement = VectorXd(3);
      double rho;
      double phi;
      double rho_dot;
      iss >> rho;
      ...
      iss >> rho_dot;
      iss >> timestamp;
      raw_meaasurement << rho, phi, rho_dot;

      measurement_pack_list.push_back(
              MeasurementPackage(raw_meaasurement, MeasurementPackage::RADAR, timestamp)
      );
    }
