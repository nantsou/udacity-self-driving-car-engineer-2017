  ...
  namespace cf {
    ...
    inline bool check_collision(const pair<Polynomial, Polynomial> &traj,
      ...
      for (int i = 0; i < time_step; i++) {
        ...
        double my_car_latest_vel_s = traj.first.eval(i, "first");
        ...
        for (auto other_car:other_cars) {
          ...
          double length_buffer = 2.5;
          if (diff_s < 0 && other_car.vel_s > my_car_latest_vel_s) length_buffer = 5;

          if ((abs(diff_s) <= car_critical_length*length_buffer) && (diff_d <= car_critical_width*2.5)) {
            return true;
          }
        }
      }
    ...
    inline double busy_lane_cost(const pair<Polynomial, Polynomial> &traj,
      ...
      if (future_closest_car_id != -1 && future_lane != current_lane) {
        ...
        if (future_diff_s < time_step*0.8) {
          ...
          if ((closest_car_id != -1) && (future_diff_s < 100)) {
