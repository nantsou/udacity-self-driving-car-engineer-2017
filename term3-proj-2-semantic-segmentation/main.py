...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    # Define 1x1 conv layers of each layer output
    ...
    # Deconv layers 7
    l7_deconv = tf.layers.conv2d_transpose(l7_conv_1x1, num_classes, 4, 2, padding='SAME',
                                           kernel_initializer=kernel_initializer)

    # Add layers 7 output to layers 4 output
    l4_sum = tf.add(l7_deconv, l4_conv_1x1)
    l4_deconv = tf.layers.conv2d_transpose(l4_sum, num_classes, 4, 2, padding='SAME',
                                           kernel_initializer=kernel_initializer)

    # Add layers 4 output to layers 3 output
    l3_sum = tf.add(l4_deconv, l3_conv_1x1)
    output = tf.layers.conv2d_transpose(l3_sum, num_classes, 16, 8, padding='SAME',
                                     kernel_initializer=kernel_initializer)
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        for batch, (image, label) in enumerate(get_batches_fn(batch_size)):
            feed_dict = {input_image: image, correct_label: label, keep_prob: .75, learning_rate: 1e-4}
            _, loss = sess.run([train_op, cross_entropy_loss], feed_dict=feed_dict)
            print("Epoch: {epoch} - Batch: {batch} - Loss: {loss:.5f}"\
                  .format(epoch=(epoch + 1), batch=batch, loss=loss))
